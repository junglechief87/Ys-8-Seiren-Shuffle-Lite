...
//	GF_MP6554_TBOX04,			//★後から追加したので別の所に定義してある
	...
	// shop rank custom flags are a good candidate for re-arch and removal in the future, there are other ways to do this... if we need space.
	GF_TBOX_DUMMY081, //Shop rank 2: shop ranks 1 is finding kathleen
	...
	GF_TBOX_DUMMY118, //sahad joins at the start of the avolodragil fight and the control for that fight is GF_01MP1103_JOIN_SAHAD, which also controls stuff that happens after the fight so we want a new flag for sahad joining for the client to track since his check is technically earlier.
	...
	GF_TBOX_DUMMY125, 
	GF_TBOX_DUMMY126, 
	GF_TBOX_DUMMY127, //Used to check if in a warden fight -- this may be able to be deprecated in the future too (if space is needed)
